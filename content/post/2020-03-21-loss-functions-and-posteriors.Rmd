---
title: Loss Functions and Posteriors
author: Dan W Joyce
date: '2020-03-21'
...
Frequently in medical applications of machine learning we see summary measures of performance used to demonstrate the competence of the model typically reported as sensitivities, specificities, balanced accuracy, area under the ROC curve and so on.  These measures assume that for an input $x$ (a patient) we get an output $F(x) = y$ representing for example, "negative" or "positive" caseness.  
...
```{r echo = FALSE}
knitr::include_graphics("/img/prediction-classification.png")
```

We are used to seeing *point estimates* as outputs from predictive models given some input e.g. $\Pr( Y = 1 | x) = 0.78$ and sometimes, with a measure of uncertainty on that output (for example, the standard error on predicted values from [`predict.glm()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/predict.glm.html) in `R`).  
...
Our question is now: what is the best **action** (candidate $a_{i}$) to choose as the point summary given the loss function $L_{0}$ and the posterior $\pi(Y|x)$?

The answer is, the $a_{i}$ that **minimises** the expected loss / risk.  Implementing this, we arrive at:
...
L_{1}(a_{i},y_{j}) = \begin{cases}
          c_{1} |a_{i} - y_{j}| & \text{ if } a \leq y_{j} \\
          c_{2} |a_{i} - y_{j}| & \text{ if } a > y_{j} \\
    \end{cases}
$$
