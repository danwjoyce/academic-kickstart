...
As above, assume we have a model $F(x)$ that delivers a continuous score $Y$ for an outcome/case/event given some "input" measurement or feature(s), $x$ for a patient.  Importantly, the model delivers outputs in the form of posterior probabilities $\Pr(Y=y|x)$;  for example, the probability of being a positive case is $\Pr(Y=1|x)$ and  $\Pr(Y=0|x) = 1 - \Pr(Y=1|x)$ being the probability of a negative case.  
